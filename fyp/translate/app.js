...
const metadataURL = "https://teachablemachine.withgoogle.com/models/Vf3pXh-Fc/metadata.json";
...
async function predictImage(file) {
    const modelURL = 'https://teachablemachine.withgoogle.com/models/Vf3pXh-Fc/';
    const model = await tmImage.load(modelURL + 'model.json', modelURL + 'metadata.json');


    const imagePreview = document.getElementsByClassName('webcam-view')[0];
...
uploadBtn.addEventListener('click', function() {
    ...
    fileInput.addEventListener('change', async(event) => {
        ...
        if (!file) navigator.mediaDevices.getUserMedia({ video: true })
    .then(function(stream) {
        const videoElement = document.createElement('video');
        videoElement.srcObject = stream;
        videoElement.play();
